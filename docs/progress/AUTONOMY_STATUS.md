...
Close only the shared string-identifier strict-wire evidence branch exercised
by the `DataObjectID` and `ProvenanceID` Codable tests without changing
production API. Both types decode through the one
`VoxeliaStringIdentifier` protocol decoder, so treat them as one tightly
related group: add `VOX-ERR-001` traceability to both tests and replace only
their broad five-fixture loops with exact evidence — the whitespace-only
`rawValue` must produce `DecodingError.dataCorrupted` with the single-element
`rawValue` coding path and underlying
`VoxeliaStringIdentifierError.emptyOrWhitespaceOnly`, the empty and extra-key
objects must produce root-context `dataCorrupted` from the exact one-key
guard, and the plain-string and array shapes must produce
`DecodingError.typeMismatch` from the keyed-container request. Record this as
shared strict-keyed identifier-wire evidence only, not identifier vocabulary
or uniqueness semantics, concrete-type rejection payloads, canonical JSON
bytes, completion of `VOX-API-004`, global `VOX-ERR-001` or `VOX-VAL-001`, or
any change to the accepted wire shape. Do not change source/API, controlled
`v0.1.1` baselines or Proposed/Draft contracts.

## Test policy for the next action

- Run only `swift test --filter codableRoundTripAndValidation`, the owning
  `swift build --target VoxeliaCore`, strict format lint for
  `Tests/VoxeliaCoreTests/DataObjectIDTests.swift` and
  `Tests/VoxeliaCoreTests/ProvenanceIDTests.swift`, requirement-index and
  release-integrity checks. Add no direct-dependant build because the public
