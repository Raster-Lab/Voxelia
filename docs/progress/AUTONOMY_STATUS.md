...
The exact next action is `ADR-0197` increment (j), the arc's last: the
`VOX-SUR-009` assessment — "geometry generated by Metal compute shall be
representable without making GPU buffers the canonical geometry model". It is
the GPU-**producer** mirror of the GPU-consumer question `ADR-0196` discharged
for `VOX-GEO-011`, but its verification methods are **I,T**, not `I,R`: unlike
`VOX-GEO-011` it carries a real test obligation and may need a built transfer
path, plausibly composing `ADR-0186`'s governed Metal shared-buffer boundary.
Whether that is a documentation assessment or a built path is that increment's
finding, and `ADR-0197` decision 7 forbids pre-judging it.
...
- Perform `ADR-0197` increment (j) next: the `VOX-SUR-009` assessment. Read the
  requirement's verification methods first — they are **I,T**, so a
  documentation-only discharge of the kind `VOX-GEO-011` licensed is **not**
  available here. Establish by inspection what a Metal-produced geometry path
  would have to look like against `ADR-0186`'s governed shared-buffer boundary
  and `ADR-0183`'s canonical-model rule, then discharge the test obligation with
  real evidence — and if the honest finding is that a built path is required,
  build it rather than narrowing the claim.
