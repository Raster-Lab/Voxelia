...
Close only the `StorageKind`/`StoragePersistence` invalid-wire evidence
branch in the owning `VoxeliaStorageTests` without changing production API.
Both are synthesized raw-value enums sharing one six-fixture loop, so treat
them as one tightly related group: add `VOX-ERR-001` traceability to the
invalid-values test and replace the broad loop with exact evidence for both
enums — the unknown token must produce root-context
`DecodingError.dataCorrupted`, `null` must produce
...
closed-vocabulary wire-rejection evidence only, not storage capability or
persistence semantics, canonical JSON bytes, completion of `VOX-API-004`,
global `VOX-ERR-001` or `VOX-VAL-001`, or any change to the accepted raw
values. Do not change source/API, controlled `v0.1.1` baselines or
Proposed/Draft contracts.

## Test policy for the next action

- Run only `swift test --filter StorageTaxonomy`, the owning
  `swift build --target VoxeliaStorage`, strict format lint for
  `Tests/VoxeliaStorageTests/StorageTaxonomyTests.swift`, requirement-index
