...
- One-hundred-eighty-fourth autonomous increment (scheduled goal continuation):
  audited the coordinate-bearing mesh ownership conflict and drafted proposed
  `ADR-0187`, without changing the package graph, product source or governance
  authority. The Requirements Baseline assigns coordinate spaces to
  `VoxeliaSpatial`, meshes to `VoxeliaGeometry` and requires geometry positions
  to declare their space. MTA/CDMS bind the full Spatial-owned
  `CoordinateSpaceDescriptor`; accepted `ADR-0043` supplies its exact
  length-unit, convention, handedness and external-reference admission. The
  RPSS/MTA graph instead declares only `VoxeliaGeometry -> VoxeliaCore`, while
  Core already depends on Spatial and deliberately does not redeclare or
  re-export its types under `ADR-0021`.

  The proposal recommends the minimal explicit resolution: retain the Core
  edge and add `VoxeliaGeometry -> VoxeliaSpatial`. Spatial remains the sole
  coordinate-space owner; Geometry remains the sole mesh owner; canonical
  positions later bind the exact descriptor rather than an identifier string,
  wrapper or alias. The added direct edge would introduce no cycle, package,
  product or new resolved module because Spatial is already transitively
  present. Acceptance would first authorise controlled correction `CCR-0027`,
  then one graph-only migration of the manifest, dynamic/static checkers,
  module/release graph documentation and direct test dependencies. The complete
  mesh aggregate and extraction numeric model remain separate design gates.

  Before drafting, `python3 Tools/Scripts/check_package_graph.py`,
  `check_package_graph_static.py` and `check_prohibited_imports.py` all passed
  the unchanged baseline graph, proving Geometry currently declares only Core.
  After adding the Proposed record and register entry, the direct ADR checker
  passed all 167 records; `Tools/Scripts/validate-docs.sh` passed seven
  front-matter documents, all 167 ADRs, the two primary and one companion Draft
  RFC records and text checks for 272 Markdown files. The same dynamic/static
  graph and prohibited-import checks passed unchanged, and `git diff --check`
  passed. No Swift target changed, so no build, test, semantic safety or Apple
  destination gate was rerun; none is claimed by this documentation-only
  increment. Release-integrity regeneration and read-only manifest/integrity
  verification passed with 818 manifest paths, 817 inventory records and 818
...
- Two-hundred-twenty-second autonomous increment (`ADR-0202` migration):
  `VoxeliaRendering` now owns `SurfaceShader`, and `SurfaceHit`,
  `SurfaceFragment` and `CoveredSample` carry the swap flag. All fourteen
  `VOXELIA-ALG-0036` fixtures reproduce both registered SHA-256 digests
  bit-exactly on the first run.

  **The additive correction was verified before anything else was built.** The
  swap flag was added to the three values and the `ALG-0034` and `ALG-0035`
  oracle tests were re-run immediately: both pairs of digests still match, so
  the claim that the correction changes no registered digest is proven rather
  than argued. Only then was the shading reference written.

  `SurfaceShader.intensity` is **non-throwing**, matching `ADR-0202` decision
  10's proof that no representability failure is reachable. The swap flag does
  real work: a dedicated test shows that setting it changes the intensity, and
  that setting it is exactly equivalent to exchanging the second and third
  weights by hand — so a consumer that ignored it would silently mis-shade
  every mirrored facet, and now fails a test instead.

  The registered numeric findings all reproduce in Swift: the clamp fires on a
  unit normal's self-projection; a least-subnormal normal underflows at one
  third weight but survives at full weight; facing and facing-away are lit
  identically, confirming the two-sided rule.

  Verified after a clean `.build` rebuild: 750 tests in 160 suites green.
  `ADR-0197` increments (a) through (e) are complete, discharging the **Test**
  half of `VOX-SUR-001` through `VOX-SUR-004`.

  ```bash
  swift test --filter 'SurfaceShaderTests|SurfaceVisibilityResolverTests|SurfaceCompositorTests'
  swift format lint --strict Sources/VoxeliaRendering/Internal/SurfaceShader.swift
  rm -rf .build && swift test
  Tools/Scripts/validate-docs.sh
  python3 Tools/Scripts/check_release_integrity.py --write
  python3 Tools/Scripts/check_release_integrity.py
...
- Two-hundred-thirtieth autonomous increment (`ADR-0206` migration):
  `VoxeliaRendering` now owns `SurfaceAnnotationRegistrar`. All nineteen
  `VOXELIA-ALG-0040` fixtures reproduce both registered SHA-256 digests
  bit-exactly on the first run.

  **The two-pose claim is proven against the accepted projector, not against
  hand-written coordinates.** One anchor fixed in world space is projected by
  `SurfaceVertexProjector` at a head-on pose and at a pose along the anchor's
  own axis: its pixel moves right of centre in the first and collapses onto the
  centre column in the second, its row is unchanged, and its depth follows the
  pose. Re-evaluating the first pose after the second returns a bit-identical
  registration, which is the statelessness claim made observable rather than
  asserted.

  **The reference deliberately has no `SurfaceVisibilityBuffer` overload, and
  the omission is recorded in the source.** That buffer is resolved without a
  clip, so consuming it directly would silently let a clipped-away surface hide
  an annotation — the exact ordering `ADR-0205` decision 3 forbids. The caller
  composes the clip and supplies the retained depth, as it does for picking. The
  registrar publishes its own pixel rule so the caller's occlusion lookup cannot
  address a different pixel than the placement did.

  The suite proves the floor rule on both sides of an exact integer and at a
  pixel centre, all four viewport edges, negative zero, and the strict unbiased
  comparison — including that one unit in the last place either side of an
  exactly equal depth decides it, which is what having no epsilon means. It also
  proves the signed-depth behaviour behind the camera in both directions.

  The stage is non-throwing and carries no failure family, as designed: the
  continuous-coordinate bound runs before any integer conversion, so the
  conversion is total.

  Verified: 764 tests in 164 suites green. `ADR-0197` increments (a) through (i)
  are complete. The **Test** half of `VOX-SUR-008` is discharged; its
  continuous-motion **demonstration** half remains gated to the owner-gated
  interactive draw loop and is not claimed.

  ```bash
  swift test --filter SurfaceAnnotationRegistrarTests
  swift format lint --strict \
    Sources/VoxeliaRendering/Internal/SurfaceAnnotationRegistrar.swift \
    Tests/VoxeliaRenderingTests/SurfaceAnnotationRegistrarTests.swift
  swift test
  Tools/Scripts/validate-docs.sh
  python3 Tools/Scripts/check_release_integrity.py --write
  python3 Tools/Scripts/check_release_integrity.py
...
- Two-hundred-thirty-first autonomous increment (`ADR-0197` increment (j),
  design): accepted `ADR-0207` and `VOXELIA-ALG-0041` assess `VOX-SUR-009` and
  freeze `gpu-geometry-representation/v1`. No product source changed.

  **The verification-method column decided the outcome, and it went the other
  way from `ADR-0196`.** That record discharged the GPU-*consumer* question
  documentation-only because `VOX-GEO-011` declares `I,R`. `VOX-SUR-009`
  declares **I,T**, so the mirror question carries a real test obligation, and
  treating the two rows as equivalent because their subject matter mirrors would
  have claimed evidence that was never produced. `ADR-0197` decision 7 forbade
  pre-judging this, and the finding is a **built path**.

  **Inspection found the transport already solved and the layout entirely
  missing.** `ADR-0186` already returns a fresh owning `[UInt8]` copied out of a
  validated shared-buffer range after the writing command buffer completes, and
  `VoxeliaGeometry` already forbids both `Metal` and (since `ADR-0196`)
  `ModelIO`. But between "owned bytes" and "canonical mesh" there was **no
  accepted contract at all** — so the first producer to arrive would have
  invented its own layout, which is precisely how a buffer's shape quietly
  becomes the geometry model.

  **The layout is the contract; the buffer is only transport.** Nothing in
  `ALG-0041` names Metal, a device, a buffer or a command queue, and the decode
  is a pure function from bytes to a canonical value — which is also why the
  **T** method is fully dischargeable with no GPU. The decoder belongs in
  `VoxeliaGeometry` and that placement *is* the architectural point: putting it
  in `VoxeliaMetal` would make the geometry path depend on whichever backend
  happens to produce the bytes today. **No Metal dependency edge is added.**

  **The decode carries no geometric rule.** Its own failure family is four
  byte-level cases; every geometric rejection — non-finite position,
  out-of-range index — belongs to the canonical value's own admission and is not
  restated. Two canonical cases are moreover structurally **unreachable**
  through this path, because an exact byte-count match is strictly stronger than
  a multiple-of-three check.

  Frozen numeric boundaries: positions are **tightly packed** three-component
  binary32 (MSL's `float3` is sixteen bytes with padding, `packed_float3` is
  twelve — the likeliest way for a conforming-looking producer to be wrong, so
  it is stated); little-endian is registered with a byte-order fixture; counts
  arrive **out of band**, because a length-bearing header would let the payload
  decide its own allocation; the **coordinate space is never carried in the
  payload**, since letting bytes name a space is the relabelling `ADR-0183`
  decision 4 forbids; and binary32-to-binary64 widening is **exact**, with the
  honest consequence registered — decimal `0.1` written as a binary32 publishes
  as `0.10000000149011612`, not as `0.1`.

  No geometry-producing compute kernel was written and none is claimed: the
  requirement asks whether such geometry is *representable*, not that a producer
  exist, and writing one with no consumer is the expansion `ADR-0196` decision 3
  already refused for the mirror question.

  The oracle registers eighteen fixtures, eight decoded and ten rejected.

  ```bash
  python3 docs/progress/evidence/ADR-0207-gpu-geometry-representation-oracle.py
  Tools/Scripts/validate-docs.sh
  python3 Tools/Scripts/check_adr_register.py
  python3 Tools/Scripts/generate_requirement_index.py --check
  git diff --cached --name-only | grep -E '^(Sources|Tests)/'
...
The `ADR-0202` migration is complete: the swap flag is published on all three
...
The `ADR-0206` migration is complete, and with it `ADR-0197` increments (a)
through (i). `VoxeliaRendering` owns `SurfaceAnnotationRegistrar`, both
`ALG-0040` digests reproduce bit-exactly, and the two-pose claim is proven
through the real `SurfaceVertexProjector`. The **Test** half of `VOX-SUR-008` is
