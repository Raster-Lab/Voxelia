...
Close only the existing `LookupTableDescriptor` strict-wire malformed-fixture
evidence branch without changing production API. Add `VOX-ERR-001`
traceability to the strict-and-revalidating decoding test and replace only its
two remaining broad assertions with exact evidence: the missing-`outputUnit`
and extra-key object fixtures must produce root-context
`DecodingError.dataCorrupted` from the exact three-key guard, the array
fixture must produce `DecodingError.typeMismatch` from the keyed-container
request, and the blank-namespace nested unit fixture must produce
`DecodingError.dataCorrupted` with the `outputUnit`/`namespace` coding path
and underlying `MeasurementUnitError.emptyNamespace` from the nested
`MeasurementUnit` decoder. Record this as strict exact-key wire,
container-shape and nested-unit revalidation evidence only, not the
already-exact non-finite-value evidence, lookup execution or interpolation
semantics, unit conversion, canonical JSON bytes, completion of `VOX-API-004`,
global `VOX-ERR-001` or `VOX-VAL-001`, or any change to the accepted wire
shape. Do not change source/API, controlled `v0.1.1` baselines or
Proposed/Draft contracts.

## Test policy for the next action

- Run only `swift test --filter decodingIsStrictAndRevalidates`, the owning
  `swift build --target VoxeliaCore`, strict format lint for
  `Tests/VoxeliaCoreTests/LookupTableDescriptorTests.swift`, requirement-index
