...
Close only the existing `Plane3D`/`Ray3D` wrong-shape evidence branch in the
owning `VoxeliaSpatialTests` without changing production API. In the
`PlaneAndRayTests` invariant-revalidation test (already `VOX-ERR-001`-tagged),
replace only the two broad wrong-shape loops with exact evidence for both
composites: the missing-key and extra-key objects must produce root-context
`DecodingError.dataCorrupted` from the exact two-key guard, and the array
shape must produce `DecodingError.typeMismatch` from the keyed-container
request. Record this as strict two-key wire and container-shape evidence
only, not the already-exact composite-invariant revalidation evidence, plane
or ray semantics, canonical JSON bytes, completion of `VOX-API-004`, global
...
- Run only `swift test --filter decodingRevalidatesInvariants`, the owning
  `swift build --target VoxeliaSpatial`, strict format lint for
  `Tests/VoxeliaSpatialTests/PlaneAndRayTests.swift`, requirement-index and
